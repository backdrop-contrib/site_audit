...
class SiteAuditCheckInsightsAnalyze extends SiteAuditCheckAbstract {
  ...
  public function getResultInfo() {
    if (empty($this->registry['url'])) {
      return t('No Pagespeed Test URL is set. See Site Audit Settings.');
    }
    return $this->getResultPass();
  }
...
//      }
      ...
      $impact_filter = $this->getOption('impact');
      ...
      foreach ($this->registry['json_result']->lighthouseResult->audits as $audit => $details) {
        $items[$audit] = array(
          '#type' => 'item',
          '#title' => $details->title,
        ...
        if (!empty($details->displayValue)) {
          $items[$audit]['value'] = array(
            '#markup' => $details->displayValue,
            '#prefix' => '<blockquote>',
            '#suffix' => '</blockquote>',
          );
        }
        ...
        if ($details->score) {
          if ($details->score < 1) {
            $items[$audit]['#title'] .= ': ' . $details->score * 100 . '%';
          }
        }

        if (!empty($this->registry['json_result']->lighthouseResult->stackPacks[0]->descriptions->{$audit})) {
          $items[$audit]['suggestion'] = array(
            '#markup' => $this->registry['json_result']->lighthouseResult->stackPacks[0]->descriptions->{$audit},
            '#prefix' => '<blockquote>',
            '#suffix' => '</blockquote>',
          );
        }
...
//      }
  ...
  public function calculateScore() {
    $pso_url = 'https://www.googleapis.com/pagespeedonline/v5/runPagespeed';
    ...
    // Overview.
    if (!empty($this->registry['json_result']->lighthouseResult->categories->performance->score)) {
      $score = $this->registry['json_result']->lighthouseResult->categories->performance->score * 100;
      if ($score > 80) {
